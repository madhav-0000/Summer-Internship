...
#### `__init__(self, mar_threshold=0.6, mar_frames=15)`
- **Configuration:** 
  - `mar_threshold`: The value above which the mouth is considered open wide enough to be a yawn.
  - `mar_frames`: The number of consecutive frames the MAR must remain above the threshold before an alarm is triggered.
  - `yawn_frames`: A counter that increments each frame the mouth is wide open.
...
- **Output:** Returns a tuple `(mar, alarm_on)`.
- **Behavior:** 
  1. Extracts the mouth coordinates.
  2. Calculates the MAR.
  3. If the MAR is above the threshold, it increments the `yawn_frames` counter. If it reaches the frame threshold, it sets `alarm_on` to `True`. Otherwise, it resets the counter.
