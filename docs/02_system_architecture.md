...
2. **Face Mesh Engine**: Processes the frame using MediaPipe to detect 468 3D facial landmarks.
3. **Trackers**:
...
4. **Alert System**: Monitors the trackers' states over successive frames. If conditions are met (e.g., EAR < threshold for N frames), triggers a sound via `pygame`.
